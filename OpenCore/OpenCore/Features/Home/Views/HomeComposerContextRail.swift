...
struct HomeComposerContextRail: View {
...
private struct HomeComposerContextUsageIndicator: View {
    ...
    var body: some View {
        ...
        .shadow(color: palette.elevationShadow(lightOpacity: 0.06), radius: 10, x: 0, y: 4)
    }
}
...
private struct HomeComposerContextUsagePopover: View {
    ...
    var body: some View {
        ...
        .shadow(color: palette.elevationShadow(lightOpacity: 0.12, darkOpacity: 0.35), radius: 14, x: 0, y: 8)
    }
...
struct HomeComposerStopRecordingButton: View {
    let action: () -> Void
            ...
            Image(systemName: "arrow.up")
                ...
                .background(
                    canSend
                        ...
                        : palette.surfaceGalaxyTint.opacity(palette.isDark ? 0.55 : 0.95)
                )
                .clipShape(Circle())
        }
        .buttonStyle(.plain)
...
private struct HomeComposerGlassChrome: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .background {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .fill(palette.isDark ? palette.surfacePaper.opacity(0.85) : palette.surfaceRaised)
            }
            .background {
                if palette.isDark {
                    RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
            ...
            .shadow(
                color: palette.elevationShadow(lightOpacity: shadowOpacity, darkOpacity: min(shadowOpacity * 2.5, 0.4)),
                radius: 18,
                x: 0,
                y: 8
            )
